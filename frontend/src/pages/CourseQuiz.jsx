  ...
  useEffect(() => {
    const fetchQuiz = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          // 404 means no quiz for this courseId
          console.warn("Quiz API not ok:", res.status);
          setQuiz(null);
          return;
        }
        ...
        // ✅ FIX: Support both backend response styles:
        // Style A: { quiz: {...} }
        // Style B: quiz object directly { course, questions, ... }
        const q = data?.quiz ?? data;

        // ✅ Ensure questions exists and is array
        if (!q || !Array.isArray(q.questions)) {
          console.warn("Quiz response format unexpected:", data);
          setQuiz(null);
          return;
        }
    ...
    return (
      <div style={{ textAlign: "center", marginTop: "2rem", color: "#ef4444" }}>
        ...
        <div style={{ marginTop: "1rem", color: "#9ca3af", fontSize: "0.9rem" }}>
          Tip: Only courses that were seeded in <b>seedQuizzes.js</b> will have quizzes.
        </div>
      </div>
    );
