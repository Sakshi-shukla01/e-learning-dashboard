...
const CourseDetails = () => {
  ...
  // ✅ NEW: store signed notes url from backend
  ...
  // ✅ NEW: fetch signed notes link (only after purchase)
  const fetchNotesLink = async () => {
    try {
      ...
      const res = await axios.get(`${API_URL}/courses/${id}/notes-link`, {
        headers: { Authorization: `Bearer ${token}` },
      });

      if (res.data?.url) {
        setNotesUrl(res.data.url);
      } else {
        alert("Notes link not available.");
      }
    } catch (err) {
      console.error("Error fetching notes link:", err.response || err);
      alert(err.response?.data?.message || "Unable to load notes link.");
    } finally {
      setNotesLoading(false);
    }
  };

  // ✅ when purchased becomes true, load notes link once
  useEffect(() => {
    if (purchased) {
      fetchNotesLink();
    }
    ...
    >
      ...
      {purchased && (
        <>
          ...
          {/* Notes Section (UPDATED for S3 signed URL) */}
          {(course.notesKey || course.notes) && (
            <section style={{ marginTop: "2rem" }}>
              <h2 style={{ fontSize: "1.5rem", fontWeight: "600", marginBottom: "1rem" }}>
                📘 Course Notes
              </h2>
              ...
              <div style={{ display: "flex", gap: "10px", marginBottom: "1rem" }}>
                <button
                  ...
                  onClick={async () => {
                    ...
                    if (!notesUrl) await fetchNotesLink();
                    if (notesUrl) window.open(notesUrl, "_blank");
                  }}
                  disabled={notesLoading}
                >
                  <FaEye /> {notesLoading ? "Loading..." : "Preview Notes"}
                </button>
                ...
                <button
                  ...
                  disabled={notesLoading}
                >
                  <FaDownload /> {notesLoading ? "Loading..." : "Download Notes"}
                </button>

                <button
                  className="btn btn-secondary"
                  onClick={fetchNotesLink}
                  disabled={notesLoading}
                  title="Signed URL expires, click to refresh"
                >
                  Refresh Link
                </button>
              </div>
              ...
              {notesUrl ? (
                ...
                <p style={{ color: "#60a5fa" }}>
                  Notes link not loaded yet. Click <b>Preview Notes</b> or <b>Refresh Link</b>.
                </p>
              )}
            </section>
          )}
